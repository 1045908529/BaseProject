...
- (UICollectionViewLayoutAttributes *)layoutAttributesForSupplementaryViewOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
  ...
  if ([kind isEqualToString:CHTCollectionElementKindSectionHeader]) {
      ...
      if (indexPath.section+1 == [cv numberOfSections]) {
          ...
          frame.origin.y = MAX(contentOffset.y - 60, frame.origin.y);
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect {
  ...
  for (i = begin; i < end; i++) {
    ...
    for (NSUInteger idx=0; idx<[attrs count]; idx++) {
        UICollectionViewLayoutAttributes *layoutAttributes = attrs[idx];
        
        if (layoutAttributes.representedElementCategory == UICollectionElementCategoryCell) {
            [missingSections addIndex:layoutAttributes.indexPath.section];  // remember that we need to layout header for this section
        }
        if ([layoutAttributes.representedElementKind isEqualToString:CHTCollectionElementKindSectionHeader]) {
            [attrs removeObjectAtIndex:idx];  // remove layout of header done by our super, we will do it right later
            idx--;
        }
    }
    ...
    [missingSections enumerateIndexesUsingBlock:^(NSUInteger idx, BOOL *stop) {
        ...
        UICollectionViewLayoutAttributes *layoutAttributes = [self layoutAttributesForSupplementaryViewOfKind:CHTCollectionElementKindSectionHeader atIndexPath:indexPath];
        if(layoutAttributes)
